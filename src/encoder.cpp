...
namespace Constants
{
    ...
    const std::string startFile = "wd_1.txt";
}
...
void extractAll(std::fstream& inpstream)
{
    ...
    else if (_flag == WV_FLAG)
    {
        ...
        for (int i = 0; i < WD_ROWS; i += skip)
            for (int j = 0; j < WD_COLS; j += skip)
                values[i][j] = wv_values[i][j];
    }
...
void countNiNj()
{
    ...
    inpstream.open(startFile);

    if (!inpstream.is_open())
    {
        std::cerr << "Error opening file " << startFile << '\n';
        return;
    }
