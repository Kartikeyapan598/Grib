...
bool GribRecord::readGribSection2_GDS(ZUFILE* file) {
    ...
    Ni  = readInt2(file);				// byte 7-8 No. of points along a latitude circle
    Nj  = readInt2(file);				// byte 9-10 No. of points along a longitude meridian
    ymin = readSignedInt3(file)/1000.0;	// byte 11-12-13 latitude of first grid point; units: millidegrees
    xmin = readSignedInt3(file)/1000.0;	// byte 14-15-16 longitude of first grid point; units: millidegrees
    resolFlags = readChar(file);		// byte 17   resolution flags
    ymax = readSignedInt3(file)/1000.0;	// byte 18-19-20 Latitude of last grid point
    xmax = readSignedInt3(file)/1000.0;	// byte 21-22-23 Longitude of last grid point
	savXmin = xmin;
	savXmax = xmax;
	savYmin = ymin;
	savYmax = ymax;

    Di  = readSignedInt2(file)/1000.0;	// byte 24-25 Longitudinal Direction Increment units: millidegrees
    Dj  = readSignedInt2(file)/1000.0;	// byte 26-27 Latitudinal Direction Increment units: millidegrees
    savDi = Di;
    savDj = Dj;
    ...
    isScanIpositive = (scanFlags&0x80) ==0;     //0x80 = 128
    isScanJpositive = (scanFlags&0x40) !=0;     //0x40 = 64
    isAdjacentI     = (scanFlags&0x20) ==0;     //0x20 = 32
