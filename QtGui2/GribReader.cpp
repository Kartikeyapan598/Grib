...
void GribReader::readGribFileContent (int nbrecs)
{
    int id = 0;
	bool end;
    g2int lskip=0,lgrib=0,iseek=0;  // float
    ...
    if (file->type == ZU_COMPRESS_BZIP) {
		...
		if (lgrib == 0)
            end = false;    // end loop at EOF or problem
		iseek = lskip + lgrib;
...
void  GribReader::copyFirstCumulativeRecord (DataCode dtc)
{
	...
	if (rec == nullptr && dateref != 0)
	{
		...
		if (rec != nullptr)
		{
            ...
            r2->setRecordCurrentDate (dateref);    // 1st dummy recording
			storeRecordInMap (r2);
		}
	}
}
...
{
    // Look for the GribRecords that surround the date
...
//---------------------------------------------------
// First GribRecord (by date) for a given type
...
GribRecord * GribReader::getRecord (DataCode dtc, time_t date)
{
    ...
    if (ls != nullptr) {
        // Searches for the first recording on the correct date
        zuint nb = ls->size();
...
//-------------------------------------------------------
// Generates the list of dates for which forecasts exist
void GribReader::createListDates()
{   // The set ensures the order and the uniqueness of the dates
    setAllDates.clear();
	for (auto const & it : mapGribRecords )
	{
		std::vector<std::shared_ptr<GribRecord>> *ls = it.second;
