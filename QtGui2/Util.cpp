...
QString Util::formatTemperature (float tempKelvin, bool withUnit)
{
    ...
    QString r;
    if (unit == tr("°K")) {
        r.asprintf("%.1f", tempKelvin);
    }
    else if (unit == tr("°F")) {
        r.asprintf("%.1f", 1.8f*(tempKelvin-273.15f)+32.0f);
    }
    else  {   // if (unit == tr("°K"))
        unit = tr("°C");
        r.asprintf("%.1f", tempKelvin-273.15f);
    }
...
QString Util::formatTemperature_short(float tempKelvin, bool withUnit)
{
    ...
    QString r;
    if (unit == tr("°K")) {
        r.asprintf("%d", qRound(tempKelvin) );
    }
    else if (unit == tr("°F")) {
        r.asprintf("%d", qRound(1.8f*(tempKelvin-273.15f)+32.0f) );
    }
    else  {   // if (unit == tr("°K"))
        unit = tr("°C");
        r.asprintf("%d", qRound(tempKelvin-273.15f) );
    }
	return (withUnit) ? r+""+unit : r;
}
//----------------------------------------------------------------
QString Util::formatSpeed (float meterspersecond, bool withUnit, QString unit)
{
    QString r;
    if (unit == "m/s") {
        r.asprintf("%.1f ", meterspersecond);
        unit = tr("m/s");
    }
    else if (unit == "m/min") {
        r.asprintf("%d ", qRound(meterspersecond*60.f));
        unit = tr("m/min");
    }
    else if (unit == "km/h") {
        r.asprintf("%.1f ", meterspersecond*3.6);
        unit = tr("km/h");
    }
    else  {   // if (unit == tr("kts"))
        r.asprintf("%.1f ", meterspersecond*3.6/1.852);
        unit = tr("kts");
    }
	return (withUnit) ? r+" "+unit : r;
}
...
QString Util::formatDirection (float angle, bool withUnit)
{
    QString unite = tr("°");
    QString r;
    r.asprintf("%d", qRound(angle +0.5f));
...
QString Util::formatPressure (float pasc, bool withUnit, int precision)
{
	...
	if (GribDataIsDef(pasc)) {
		if (precision > 0)
            r.asprintf("%.1f", pasc/100.0f);
		else
            r.asprintf("%.0f", pasc/100.0f);
	}
	return (withUnit) ? r+" "+unite : r;
}
//----------------------------------------------------------------
...
QString Util::formatDegres (float x, bool inf100)     // 123.4 -> 123°24.00'
{
    ...
    if (unit == tr("dd°mm,mm'"))
    {
        ...
        char sign = (x<0) ? '-' : ' ';
        if (inf100)
            r.asprintf("%c%02d%s%05.2f'", sign,deg,cdeg, min);
		else
            r.asprintf("%c%03d%s%05.2f'", sign,deg,cdeg, min);
    }
    else if (unit == tr("dd°mm'ss\""))
    {
        ...
        char sign = (x<0) ? '-' : ' ';
        if (inf100)
            r.asprintf("%c%02d%s%02d'%02d\"", sign,deg,cdeg, min,sec);
		else
            r.asprintf("%c%03d%s%02d'%02d\"", sign,deg,cdeg, min,sec);
    }
    else // if (unit == tr("dd,dd°"))
    {
        if (inf100)
            r.asprintf("%05.2f%s",x,cdeg);
		else
            r.asprintf("%06.2f%s",x,cdeg);
    }
    return r;
}
//---------------------------------------------------------------------
