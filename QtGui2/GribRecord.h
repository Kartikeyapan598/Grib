...
class GribRecord : public RegularGridRecord  
{
    public:
        ...
        //-----------------------------------------
        void    translateDataType();  // adapts the codes of the different weather centers
        //-----------------------------------------
        ...
        // Is the value defined (grid with holes)?
        inline bool   hasValue (int i, int j) const;

        // Reference date (file creation)
    ...
    protected:
        ...
        bool   ok{false};    // data validity
        bool   knownData; 	// known data type
        bool   waveData{false};
		
        bool   eof;   // end of file reached while reading
		uint64_t dataKey;
		char   strRefDate [32];
		char   strCurDate [32];
		bool   *boolBMStab{};
